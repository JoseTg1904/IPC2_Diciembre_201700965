...
class Estus_Info extends React.Component {
    ...
    obtenerDatos() {
        fetch("http://localhost:4000/Crear_Estu/")
            .then(res => res.json())
            .then(data => {
                this.ordenarAsc(data, 'universidad')
                for(var i=0;i<data.length;i++){
                    if(data[i]['pago'] === true){
                    ar.push({universidad:data[i]['universidad']})
                    }
                }
                this.depurarNac(ar)
            });
    ...
    depurarNac(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            for (var j = (i + 1); j < data.length; j++) {
                if ((data[i]['universidad'] === data[j]['universidad'])) {
                   data[i]['universidad'] = "null"
                }
            }
        }
        for (var h = 0; h < data.length; h++) {
            if (!(data[h]['universidad'] === "null")) {
                auxi.push(data[h])
            }
        }
    ...
    buscrIndividual(encargado) {
        ...
        for (var i = 0; i < aux.length; i++) {
            if (aux[i]['universidad'] === encargado) {
                auxi.push(aux[i])
            }
        }
                    ...
                    <div>
                        <select className="form-control" id="seleccion">{
                           this.state.nacs.map(admin => {
                                return (
                                    <option>{admin.universidad}</option>
                                )
                            })
                        }
                        </select>
